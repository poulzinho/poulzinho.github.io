...
export default function HeroCard() {
  ...
  return (
    <BentoCard colSpan={4} rowSpan={2} variant='hero' label={t('paul_name')} className='relative'>
      {/* Background gradient */}
      ...
      <div
        className={`absolute right-6 top-2 select-none font-black leading-none tracking-tighter ${s.stat}`}
      ...
      <div className='relative flex h-full flex-col justify-between'>
        ...
        <div>
          <p className={`font-mono text-[10px] uppercase tracking-[0.25em] ${s.eyebrow}`}>
            {t('cv_hero_eyebrow')}
          </p>
          <h2 className={`mt-3 font-black leading-[0.92] tracking-tighter ${s.name}`}
            style={{ fontSize: 'clamp(2.5rem, 6vw, 3.75rem)' }}
          >
            {t('paul_name').split(' ').map((word, i) => (
              <span key={i} className='block'>{word}</span>
            ))}
          </h2>
          <p className={`mt-2 font-mono text-[11px] uppercase tracking-[0.18em] ${s.eyebrow}`}>
            {t('cv_hero_role')}
          </p>
        </div>
        ...
        <div>
          <p className={`text-xl leading-snug sm:text-2xl ${s.tagline}`} style={SERIF}>
            <em>{t('cv_hero_serif_tagline')}</em>
          </p>
          <div className={`my-4 h-px w-10 ${s.divider}`} />
          <p className={`max-w-sm text-sm leading-relaxed ${s.bio}`}>
            {t('cv_hero_short_bio')}
          </p>
          <p className={`mt-2 font-mono text-[10px] uppercase tracking-[0.2em] ${s.statLabel}`}>
            {t('cv_hero_years')} {t('cv_hero_years_label')} · {t('cv_hero_location_short')}
          </p>
        </div>
        ...
        <div className='flex flex-col gap-3 pt-4 sm:flex-row sm:items-center sm:justify-between'>
          ...
          <p className={`font-mono text-[10px] uppercase tracking-[0.2em] ${s.availability}`}>
