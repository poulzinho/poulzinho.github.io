...
function ControlsContent({
  ...
  autoBoost,
  onAutoBoostChange,
}: Omit<Props, 'isOpen' | 'onOpenChange'>) {
            ...
            {SLIDERS.map(({ key, label, min, max, step }) => (
              ...
              >
                ...
                <input
                  ...
                  onChange={e => onChange(i, key, parseFloat(e.target.value))}
...
export default memo(function WaveControls({
  ...
  const contentProps = { configs, onChange, onDirectionChange, onReset, dynamic, onDynamicChange, autoBoost, onAutoBoostChange }
  ...
  return (
    <>
      ...
      <>
        ...
        {/* Sheet */}
        <div
          className={`fixed bottom-0 left-0 right-0 z-50 rounded-t-2xl bg-black/80 backdrop-blur-md transition-transform duration-300 ease-out md:hidden ${
        ...
        >
          ...
          <div className='max-h-[70dvh] overflow-y-auto px-5 pb-8 pt-2'>
            <ControlsContent {...contentProps} />
          </div>
        </div>
      </>
